...
# code written by Alex Arkilanian and Gaurav Baruah
...
slow.forcing.trans.res0.25<-subset(merged, strength=="slow" & 
                                     resolution == 0.25 & length==85 & method =="trans")
...
### ---- extracting median values for the different time series resolutioin see supplementary figure


medians <- aggregate(tau ~  resolution*metric*strength*method, data.resolution, median)
SD.resolutions<-aggregate(tau ~  resolution*metric*strength*method, merged, var)
sim.res <- ggplot(data = data.resolution, aes(x = resolution, y = tau, fill = method:metric)) +
  geom_hline(yintercept = 0, col = 'gray', size = 0.5) +
  geom_point(aes(col = method:metric), position = position_jitterdodge(dodge.width = 0.7), alpha = 0.4, size = 0.2) +
  geom_boxplot(outlier.shape = NA, alpha = 0.5, lwd = 0.2) + 
  geom_text(data = medians, aes(resolution, tau, label = round(tau,2)), 
            position = position_dodge(width = 0.8), size = 2, vjust = -0.5)+
