# This script runs a population simulation and generates abundance data
# on which EWS analyses are performed. This script outputs a RData file 
# containing the simulated population abundances.
#
# Author:  Alex Arkilanian & Gaurav Baruah
...
r <- 2.35     # Population growth rate
K <- 100      # Carrying capacity
h <- 1        # Half-saturation constant

c.list.fold <- c(0, 0.70, 1.20, 1.70)  # Forcing parameters for fold model
c.list.trans <- c(0, 0.03, 0.05, 0.07) # Forcing parameters for transcritical model

dt.list <- c(0.025, (2 * 0.25), (2 * 0.5), (2 * 1), (2 * 2)) # List of resolutions as fractions of generation time of 2

tmax <- 100 # Max length of time series
tmin <- 0   # Min length of time series

reps <- 100 # Number of replicates to run

## Setup simulation #####

# Number of data points within each time step for each forcing level
p.list <- c((-tmin + tmax)/dt.list[1] + 1,        # real-time
            floor((-tmin + tmax)/dt.list[2] + 1), # 1/4 generation time
            floor((-tmin + tmax)/dt.list[3] + 1), # 1/2 generation time
            floor((-tmin + tmax)/dt.list[4] + 1), # 1 generation time
            floor((-tmin + tmax)/dt.list[5] + 1)) # 2 generaton times

# Forcing in order for fold model
c.fold <- c(rep(c.list.fold, each = p.list[1]))

# Forcing in order for transcritical model
c.trans <- c(rep(c.list.trans, each = p.list[1]))

# Time values (in number of timesteps)
time <- c(rep(seq(tmin, tmax, dt.list[1]), length(c.list.fold)))

# Logical vectors marking when to reinitialize population size to N.init
reInit <- time == tmin
...
## Running model at dt = dt.list[1] and subsetting to low-res #####
